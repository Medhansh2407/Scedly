...
function Footer() {
  return <><section className="border-t border-border bg-bg-elev py-24 text-center"><div className="mx-auto max-w-content px-5"><span className="font-mono text-xs uppercase tracking-[.18em] text-accent">Your time, expressed as intent</span><h2 className="mt-4 text-[clamp(2rem,4vw,3.25rem)] font-[700] leading-tight tracking-tight text-text-primary">Stop arranging blocks.<br/>Start describing what needs to happen.</h2><p className="mx-auto mt-4 max-w-[56ch] text-lg text-text-secondary">Scedly translates the rest into a schedule—and stays with the plan when the day moves.</p><Link href="/demo" className="mt-7 inline-flex rounded-lg bg-accent px-6 py-3 font-semibold text-[#04121f]">Try the guided demo →</Link></div></section><footer className="border-t border-border py-10"><div className="mx-auto flex max-w-content flex-col gap-6 px-5 md:px-6"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center"><Link href="/" className="font-mono font-bold text-text-primary">scedly<span className="animate-blink text-term-green">_</span></Link><div className="flex flex-wrap gap-5 text-sm text-text-secondary">{nav.map(([l,id])=><a key={id} href={`#${id}`}>{l}</a>)}<Link href="/demo">Demo</Link><Link href="/login">Sign in</Link></div></div><div className="h-px bg-border"/><div className="flex flex-col justify-between gap-3 font-mono text-xs text-text-tertiary sm:flex-row"><span>© Scedly. All rights reserved.</span><span>Natural-language scheduling that adapts.</span></div></div></footer></>;
}
